...
public final class SoundTouch {
    ...
    private native final int processFrame(long handle, byte[] sampleBuffer, int sampleRate, int nChannels);
    ...
    public int processFrame(byte[] buffer, int sampleRate, int chanels) {
        return processFrame(handle, buffer, sampleRate, chanels);
    }

    int count = 0;

    public void receiveSamples(byte[] buffer) {
        count += buffer.length;
        Log.e("receiveSamples", "receiveSamples=" + buffer.length + " count=" + count);
        receiveCallBack.onReceive(buffer);
    }
